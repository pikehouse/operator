...
must_haves:
  truths:
    ...
    - "shell() is a pure execution function (no internal logging)"
    - "SessionAuditor logs tool calls with timestamps and reasoning to JSON"
    - "SessionAuditor.log_tool_call() accepts shell() parameters and results for logging"
  ...
  artifacts:
    ...
    - path: "packages/operator-core/src/operator_core/agent_lab/tools.py"
      provides: "shell() pure execution tool"
      exports: ["shell"]
    - path: "packages/operator-core/src/operator_core/agent_lab/audit.py"
      provides: "Session-based audit logging (integration point for Phase 31 agent loop)"
  ...
  key_links:
    ...
    - from: "agent_lab/audit.py"
      ...
      via: "session save"
      pattern: "json\\.dumps"
    - from: "Phase 31 agent loop"
      to: "SessionAuditor.log_tool_call()"
      via: "logs each shell() execution with parameters and results"
      pattern: "deferred to Phase 31"
---
...
**Architecture Note:** shell() is a **pure execution function** — it executes commands and returns results, but does not perform audit logging internally. SessionAuditor is designed to be used by the **agent loop (Phase 31)** which logs the full conversation including tool calls. The ROADMAP criterion "shell() tool logs to audit format" means shell() executions ARE logged (by the agent loop calling `auditor.log_tool_call()` after each shell() execution), not that shell() itself performs the logging.
</objective>
...
- Reasoning parameter is not validated but required for signature (logged by caller, not by shell())
- Handle decode errors gracefully (use errors='replace')
- shell() is a **pure execution function** — no internal logging, no SessionAuditor dependency
- Phase 31 agent loop will call SessionAuditor.log_tool_call() after each shell() execution
...
- **Integration point for Phase 31 agent loop** — called after each shell() execution
- Append to messages list with:
  - timestamp (ISO format)
  - type: "tool_call"
  - tool: tool_name
  - parameters: parameters (includes command and reasoning from shell() call)
  - result: result (the structured dict returned by shell())
...
- shell() returns {stdout, stderr, exit_code, timed_out} dict (pure execution, no logging)
- SessionAuditor saves conversation history to timestamped JSON files
- SessionAuditor.log_tool_call() ready for Phase 31 agent loop integration
