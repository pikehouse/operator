...
must_haves:
  truths:
    ...
    - "RateLimiterSubject.update_limit() calls management API"
...
Output:
- RateLimiterSubject with observe() returning dict and action methods (reset_counter, update_limit)
...
3. Implement `def get_action_definitions() -> list[ActionDefinition]`:
   ...
   - Define update_limit action:
     - name: "update_limit"
     - description: "Update the rate limit for a specific key"
     - parameters: {
         "key": ParamDef(type="str", description="Rate limit key to update", required=True),
         "new_limit": ParamDef(type="int", description="New limit value", required=True)
       }
     - risk_level: "medium"
...
5. Implement `async def update_limit(key: str, new_limit: int) -> None`:
   - Call `await self.ratelimiter.update_limit(key, new_limit)`
   - Note: This requires Plan 03 to add the endpoint to ratelimiter-service

6. Add docstrings following TiKVSubject style (important for AI reasoning)
  </action>
  <verify>
```bash
...
assert hasattr(RateLimiterSubject, 'update_limit')
print('Required methods exist')
"
```
  </verify>
  <done>RateLimiterSubject implements observe(), get_action_definitions(), reset_counter(), and update_limit() following TiKV patterns</done>
</task>
...
<success_criteria>
- RateLimiterSubject.observe() returns dict with nodes, counters, node_metrics, redis_connected
- RateLimiterSubject.get_action_definitions() returns ActionDefinitions for reset_counter and update_limit
