...
must_haves:
  ...
  artifacts:
    ...
    - path: "packages/operator-core/src/operator_core/actions/exceptions.py"
      provides: "TOCTOU exception classes"
      exports: ["ApprovalExpiredError", "StateChangedError", "InvalidTokenError"]
...
<task type="auto">
  <name>Task 1: Add approval_token and version fields to ActionProposal and schema</name>
  <files>
packages/operator-core/src/operator_core/actions/types.py
packages/operator-core/src/operator_core/db/schema.py
packages/operator-core/src/operator_core/db/actions.py
...
4. Update approve_proposal() to generate approval_token using secrets.token_urlsafe(32):
   ```python
   import secrets
...
<task type="auto">
  <name>Task 2: Add optimistic locking database operation</name>
  <files>packages/operator-core/src/operator_core/db/actions.py</files>
  <action>
Add new method update_proposal_status_with_version() that checks version in WHERE clause:
...
async def update_proposal_status_with_version(
    ...
    This implements optimistic locking to detect concurrent modifications.
    ...
    Args:
        ...
        expected_version: Version the caller expects (from their last read)

    Returns:
        True if update succeeded (version matched), False if version mismatch
    """
    async with aiosqlite.connect(self.db_path) as conn:
        cursor = await conn.execute(
            ...
            """,
            (status.value, datetime.now().isoformat(), proposal_id, expected_version)
        )
        await conn.commit()

        # rowcount == 1 means update succeeded (version matched)
        # rowcount == 0 means version mismatch (concurrent modification)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        async with ActionDB(db_path) as db:
            ...
            success = await db.update_proposal_status_with_version(
                created.id, ActionStatus.VALIDATED, initial_version
            )
            assert success == True
            print('Correct version update: PASS')

            # Update with stale version should fail
            success = await db.update_proposal_status_with_version(
                created.id, ActionStatus.EXECUTING, initial_version  # Stale!
            )
            assert success == False
            print('Stale version rejected: PASS')

            # Verify version was incremented
            fetched = await db.get_proposal(created.id)
            assert fetched.version == initial_version + 1
            print('Version incremented: PASS')

asyncio.run(test())
"`
  </verify>
  <done>Optimistic locking operation implemented; correctly detects concurrent modifications via version mismatch.</done>
</task>

<task type="auto">
  <name>Task 3: Create TOCTOU exception classes</name>
  <files>packages/operator-core/src/operator_core/actions/exceptions.py</files>
  <action>
Create new file exceptions.py with TOCTOU-related exception classes:

```python
"""
Exception classes for TOCTOU defense and approval workflow (SAFE-01, SAFE-02).
"""
...
class StateChangedError(Exception):
    """Raised when system state changed between approval and execution (SAFE-01)."""
...
```
  ...
  <verify>
Run: `cd /Users/jrtipton/x/operator/packages/operator-core && python -c "
from operator_core.actions.exceptions import ApprovalExpiredError, StateChangedError, InvalidTokenError

# Test exception creation and attributes
e1 = ApprovalExpiredError(proposal_id=1, age_seconds=75.5)
assert e1.proposal_id == 1
assert e1.age_seconds == 75.5
assert '75.5' in str(e1)
print('ApprovalExpiredError: PASS')

e2 = StateChangedError(proposal_id=2, reason='concurrent_modification')
assert e2.proposal_id == 2
assert e2.reason == 'concurrent_modification'
print('StateChangedError: PASS')

e3 = InvalidTokenError(proposal_id=3)
assert e3.proposal_id == 3
print('InvalidTokenError: PASS')

print('All exception classes: PASS')
"`
  </verify>
  <done>TOCTOU exception classes created with proper attributes for error handling.</done>
</task>

<task type="auto">
  <name>Task 4: Implement TOCTOU-resistant execute_proposal with lock and expiration</name>
...
2. Add asyncio.Lock in __init__:
   ```python
   def __init__(self, ...):
       # ... existing code ...
...
3. Add token expiration check helper:
   ```python
           ...
           async with ActionDB(self.db_path) as db:
               ...
               if self._requires_approval(proposal):
                   ...
                   if self._is_approval_expired(proposal.approved_at):
                       age = (datetime.now(timezone.utc) - proposal.approved_at.replace(tzinfo=timezone.utc)).total_seconds()
                       await self._auditor.log_event(AuditEvent(
                           proposal_id=proposal_id,
                           event_type="toctou_blocked",
                           event_data={"reason": "token_expired", "age_seconds": age},
                           actor="system",
                       ))
                       raise ApprovalExpiredError(proposal_id, age)
...
from operator_core.actions.exceptions import ApprovalExpiredError, InvalidTokenError, StateChangedError
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    assert hasattr(executor, '_is_approval_expired')
    print('Executor has _is_approval_expired: PASS')
"`
  </verify>
...
<task type="auto">
  <name>Task 5: Test concurrent modification detection (optimistic locking verification)</name>
  <files></files>
  <action>
Write and run a test that verifies the optimistic locking key link works correctly under concurrent access:

1. Create a proposal and get it to VALIDATED state
2. Approve it and get the token
3. Simulate two concurrent execution attempts:
   - Read proposal in both "threads"
   - Both try to update with same version
   - Verify only one succeeds, other gets StateChangedError

This test validates the key_link: executor -> db version check.
  </action>
  <verify>
Run: `cd /Users/jrtipton/x/operator/packages/operator-core && python -c "
import asyncio
...
async def test_concurrent_modification():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Setup: Create and validate a proposal
        async with ActionDB(db_path) as db:
            proposal = ActionProposal(action_name='test', reason='test')
            created = await db.create_proposal(proposal)
            await db.update_proposal_status(created.id, ActionStatus.VALIDATED)
        ...
        # Read proposal from two 'perspectives' (simulating concurrent reads)
        async with ActionDB(db_path) as db:
            proposal_read1 = await db.get_proposal(proposal_id)
        async with ActionDB(db_path) as db:
            proposal_read2 = await db.get_proposal(proposal_id)

        version_read1 = proposal_read1.version
        version_read2 = proposal_read2.version

        print(f'Both reads got version: {version_read1}')
        assert version_read1 == version_read2

        # First update succeeds
        async with ActionDB(db_path) as db:
            success1 = await db.update_proposal_status_with_version(
                proposal_id, ActionStatus.EXECUTING, version_read1
            )
        print(f'First update succeeded: {success1}')
        assert success1 == True

        # Second update fails (stale version)
        async with ActionDB(db_path) as db:
            success2 = await db.update_proposal_status_with_version(
                proposal_id, ActionStatus.EXECUTING, version_read2  # Same stale version!
            )
        print(f'Second update succeeded: {success2}')
        assert success2 == False  # MUST fail due to version mismatch

        # Verify version was incremented only once
        async with ActionDB(db_path) as db:
            final = await db.get_proposal(proposal_id)
        print(f'Final version: {final.version}')
        assert final.version == version_read1 + 1

        print('\\nCONCURRENT MODIFICATION DETECTION: PASS')
        print('Optimistic locking key link verified: executor -> db version check works')

asyncio.run(test_concurrent_modification())
"`
  </verify>
  <done>Optimistic locking correctly detects concurrent modifications; only first update succeeds, second gets version mismatch.</done>
</task>

<task type="auto">
  <name>Task 6: Update audit logging with dual identity</name>
...
4. Optimistic locking detects concurrent modifications (verified with explicit race condition test)
...
- Concurrent modification raises StateChangedError (verified by Task 5 race test)
