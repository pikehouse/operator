...
5. **Test detection highlighting (REQUIRED - core success criterion):**
   - Kill a TiKV node: `docker stop tikv0`
   - Wait for monitor to detect (5-10 seconds)
   - **MUST verify:** Panel border changes from cyan to RED
   - **MUST verify:** Down node shows red indicator (✗)

6. **Restore node and verify recovery:**
   - Restore node: `docker start tikv0`
   - Wait for monitor to report "all passing" (5-10 seconds)
   - **MUST verify:** Panel border returns to cyan
   - **MUST verify:** Node indicator returns to green (●)

7. **Exit cleanly:** Press Ctrl+C
   - Verify: No errors, "TUI shutdown complete" message

**Expected output:**
- Cluster panel with 6 nodes, all green when healthy
- Detection highlighting (red border) MUST appear when monitor detects violations
- Detection highlighting MUST clear when cluster returns to healthy
