...
must_haves:
  truths:
    ...
    - "Existing run_campaign function remains unchanged for backward compatibility"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    IMPORTANT: The existing run_campaign() function MUST remain unchanged for backward compatibility with existing CLI usage.
    ...
    2. Update run_trial() signature to accept optional chaos_params:
       ```python
       async def run_trial(
           subject: EvalSubject,
           chaos_type: str,
           ...
           chaos_params: dict[str, Any] | None = None,  # NEW parameter
       ) -> Trial:
       ```

    3. In run_trial(), update chaos injection to pass params:
       ```python
       # Inject chaos (with params if provided)
       chaos_metadata = await subject.inject_chaos(chaos_type, **(chaos_params or {}))
       ```

    4. In run_trial(), add cleanup_chaos call AFTER final_state capture but BEFORE returning Trial:
       ```python
       # Existing code: capture final state
       final_state = await subject.capture_state()
       ended_at = now()

       # NEW: Cleanup chaos AFTER final_state capture, BEFORE building Trial
       ...
       # Existing code: build and return Trial
       return Trial(
       ...
       NOTE: Do NOT wrap this in a finally block at function level - we want to capture final_state first.
       The cleanup is deliberately placed BETWEEN final_state capture and Trial construction.

    5. Add run_campaign_from_config() as a NEW function (does NOT replace existing run_campaign):
       ```python
  ...
  <verify>
    python -c "from eval.runner.harness import run_campaign_from_config, run_campaign; print('both run_campaign functions available')"
  </verify>
  <done>
    Harness has NEW run_campaign_from_config() for YAML campaigns; existing run_campaign() unchanged; run_trial() accepts chaos_params and calls cleanup_chaos after final_state capture
  </done>
</task>
...
5. Verify existing run_campaign still works (backward compatibility):
   ```python
   from eval.runner.harness import run_campaign
   # Should import without errors - function unchanged
   ```
</verification>
...
- run_campaign_from_config() is a NEW function for YAML campaigns
- Existing run_campaign() function remains UNCHANGED (backward compatibility)
- run_trial() accepts chaos_params and calls cleanup_chaos() AFTER final_state capture
