...
must_haves:
  truths:
    ...
    - "Developer can run 'eval analyze <campaign_id> --commands' to include command analysis with destructive count"
  ...
  key_links:
    - from: "eval/src/eval/cli.py"
      ...
      via: "analyze_campaign for campaign analysis (with include_command_analysis flag)"
    ...
    - from: "eval/src/eval/analysis/scoring.py"
      to: "eval/src/eval/analysis/commands.py"
      via: "analyze_commands() called by score_trial_with_commands() when --commands flag used"
      pattern: "from eval.analysis.commands import analyze_commands"
---
