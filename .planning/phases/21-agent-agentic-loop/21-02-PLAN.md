...
<task type="auto">
  ...
  <action>
Update the `agent start` CLI command to read OPERATOR_SAFETY_MODE and OPERATOR_APPROVAL_MODE environment variables and configure executor accordingly.

Current behavior: Agent CLI creates AgentRunner but may not properly configure executor with safety mode from environment.
New behavior: Agent CLI reads env vars and creates executor with correct SafetyMode and approval_mode.
...
2. Check for OPERATOR_APPROVAL_MODE env var and convert to boolean:
```python
approval_mode_str = os.environ.get("OPERATOR_APPROVAL_MODE", "false")
approval_mode = approval_mode_str.lower() == "true"
```

3. Create SafetyController with this mode:
```python
...
4. Create ActionExecutor with explicit approval_mode from env var:
```python
...
executor = ActionExecutor(
    ...
    safety=safety,
    auditor=auditor,
    approval_mode=approval_mode,  # Explicitly from OPERATOR_APPROVAL_MODE env var
)
```

5. Pass executor to AgentRunner:
```python
runner = AgentRunner(
    subject=subject,
    db_path=db_path,
...
6. Log the mode for visibility:
```python
print(f"Agent starting in {safety_mode.value.upper()} mode (approval_mode={approval_mode})")
```

Note: The agent CLI may already create an executor. If so, ensure it respects both OPERATOR_SAFETY_MODE and OPERATOR_APPROVAL_MODE env vars.

Do NOT:
- Remove observe-only mode option (should still work when env not set)
- Hardcode approval_mode (must read from OPERATOR_APPROVAL_MODE env var)
...
- Reads OPERATOR_APPROVAL_MODE from environment
- Creates SafetyController with correct mode
- Creates ActionExecutor with approval_mode explicitly set from env var
- Passes executor to AgentRunner
- Logs the mode at startup
  </verify>
  <done>
Agent CLI honors OPERATOR_SAFETY_MODE and OPERATOR_APPROVAL_MODE environment variables and creates executor with correct mode (DEMO-01).
  </done>
</task>
